...
| `USE_TRENDS` | `false` | Comma cycle: `trends` (or `true`) = word from X trends; `pool` = AI word + random theme bank (German→English); `false`/`off`/`strategy` = AI word + strategy topic. Same index as `TWEET_STYLE` / `IMAGE_STYLE`. Example: `trends,trends,pool,pool,trends` |
